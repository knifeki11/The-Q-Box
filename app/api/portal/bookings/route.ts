...
export async function GET() {
  try {
    ...
    const stationIds = [
      ...new Set(
        [
          ...(upcomingRows ?? []).map((b) => b.station_id),
          ...(pastRows ?? []).map((b) => b.station_id),
        ].filter(Boolean)
      ),
    ];
    const { data: stations } =
    ...
    const mapBooking = (b: (typeof upcomingRows)[0]) => ({
      id: b.id,
    ...
    const past = (pastRows ?? []).map(mapBooking);

    return NextResponse.json({ upcoming, past });
  } catch (e) {
    console.error("[portal/bookings] Error:", e);
...
export async function POST(request: Request) {
  try {
    ...
    const { data: stationsOfType, error: stationsError } = await supabase
      .from("stations")
    ...
    if (insertError) {
      console.error("[portal/bookings] POST insert error:", insertError);
      return NextResponse.json(
    ...
    return NextResponse.json({ ok: true, id: booking.id, station_name: station.name });
  } catch (e) {
    console.error("[portal/bookings] POST Error:", e);
    return NextResponse.json(
